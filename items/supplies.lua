...
SMODS.ConsumableType{
	...
	primary_colour = G.C.ZBS.SUPPLYPRIMARY,
	secondary_colour = G.C.ZBS.SUPPLYSECONDARY,
	collection_rows = {4,4},
...
SMODS.Consumable {
	...
	use = function(self, card, area, copier)
		for i = 1, math.min(#jokerHighlighted(), card.ability.max_highlighted) do
			...
			G.E_MANAGER:add_event(Event({trigger = 'after',delay = 0.1,func = function()
				print(jokerHighlighted()[1].edition)
				if jokerHighlighted()[1].edition then
					for i,v in pairs(jokerHighlighted()[1].edition) do
						print(i,type(v),v)
					end
				end
				if jokerHighlighted()[1].edition and jokerHighlighted()[1].edition.negative then
					jokerHighlighted()[1]:set_edition({ zbs_zbsnegacompressed = true })
				else
